...
#if defined(mingw32_HOST_OS)
   dnsNewResolverResource   _ = return (constantResource ())
#endif
...
--
mockResolveDomainAddresses :: forall s. MockRoots -> IOSim s (Map DomainAddress (Set SockAddr))
mockResolveDomainAddresses (MockRoots localRootPeers dnsMap stdGen) = do
      ...
      resolveDomainAddresses tracerTracePublicRoots
                             ...
                             DNSResolver.defaultResolvConf
                             (mockDNSActions @Failure dnsMap genVar)
                             [ domain | (_, m) <- localRootPeers, RelayDomain domain <- Map.keys m]
...
selectPublicRootPeersEvents :: [(Time, TestTraceEvent Failure)] -> [(Time, TracePublicRootPeers)]
...
selectPublicRootResultEvents :: [(Time, TracePublicRootPeers)] -> [(Time, (Domain, [IPv4]))]
...
prop_resolveDomainAddresses_resolvesDomainsCorrectly mockRoots@(MockRoots _ dnsMap _) =
    within 1000000 $
      lookupLoop mockRoots dnsMap === dnsMap
  where
    -- Perform public root DNS lookup until no failures
